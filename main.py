...
# --- 1. UPDATED STATUS LIST (With Percentages & "Started") ---
STATUSES = [
    (0, "0% - Job Created"),
...
class DbManager:
    ...
    def login(self, username, password):
        if not self.client: return False, None
        try:
    ...
    def create_job(self, data, user):
        ...
        try:
            ...
            self.log_history(new_id, data['job_code'], None, 0, user)
            return True
        except Exception as e:
            print(f"Create Error: {e}")
            return False

    def update_job(self, job_id, data, user, job_code, old_status=None, new_status=None):
        if not self.client: return False
        try:
            self.client.table("memo_system").update(data).eq("id", job_id).execute()
            if old_status is not None and new_status is not None and old_status != new_status:
                self.log_history(job_id, job_code, old_status, new_status, user)
            return True
        except Exception as e:
            print(f"Update Error: {e}")
            return False

    def log_history(self, job_id, job_code, old_s, new_s, user):
        if not self.client: return
        try:
            log = {
                "job_id": job_id,
...
def main(page: ft.Page):
    ...
    def get_drawer():
        ...
        for idx, label in STATUSES:
            if idx < 7: 
                nav_items.append(
                    ft.NavigationDrawerDestination(icon=ft.Icons.CIRCLE_OUTLINED, label=label)
                )
    ...
    def reload_current_view():
        ...
        if view_type == "overview":
            ...
            s_idx = state["last_status_idx"]
            load_job_list_view(STATUS_DICT.get(s_idx, "Status View"))
        elif view_type == "archive":
            load_job_list_view("Archived Jobs")
        elif view_type == "search":
            term = state["last_search_term"]
            load_job_list_view(f"Results: {term}", is_global_search=True)
        else:
            load_job_list_view("Overview Dashboard")
    ...
    def show_login():
        ...
        if not users_list:
            dropdown_options = [ft.dropdown.Option("admin")]
        else:
            dropdown_options = [ft.dropdown.Option(u) for u in users_list]
        ...
        err_lbl = ft.Text(connection_status, color="grey")

        def attempt_login(e):
            if not user_dropdown.value:
                err_lbl.value = "Please select a user."
                page.update()
                return

            err_lbl.value = "Checking..."
            page.update()
            
            try:
                success, role = db.login(user_dropdown.value, pass_in.value)
                if success:
                    ...
                    err_lbl.value = "Invalid Login"
                    err_lbl.color = "red"
                    page.update()
            except Exception as ex:
                err_lbl.value = f"Error: {ex}"
                page.update()
    ...
    def show_search_view():
        ...
        page.add(
            ft.Container(
                content=ft.Column([
                    ft.Row([search_box, ft.IconButton(ft.Icons.SEARCH, on_click=run_search)]),
                    ft.Divider()
                ]),
                padding=10
            )
    ...
    def load_job_list_view(title, is_global_search=False):
        ...
        if state["role"] == "admin":
             appbar_actions.append(
                 ft.IconButton(ft.Icons.ADD, on_click=lambda e: show_job_details(None), tooltip="Create New Job")
             )
        
        appbar_actions.append(
            ft.IconButton(ft.Icons.REFRESH, on_click=lambda e: reload_current_view(), tooltip="Refresh")
        )
        ...
        # --- DASHBOARD LOGIC (When in Overview) ---
        if state["last_view_type"] == "overview":
            ...
            for j in jobs:
                ...
                if s_idx in stats:
                    stats[s_idx]['total'] += 1
                    is_flag = (j.get('flagged', 0) == 1) or (j.get('flagged') is True)
                    if is_flag:
                        stats[s_idx]['flagged'] += 1

            dashboard_col = ft.Column(spacing=10, scroll=ft.ScrollMode.ADAPTIVE, expand=True)
            
            dashboard_col.controls.append(
                ft.Container(
                    content=ft.Text(f"Total Active Jobs: {len(jobs)}", size=20, weight="bold", color="blue"),
                    padding=10
                )
            )
                ...
                card_content = ft.Container(
                    content=ft.Row([
                        ...
                        ft.Column([
                            ...
                            ft.Text(f"{total} Jobs Pending", color="grey", size=13),
                        ], alignment="center"),

                        # Right: Flag Icon
                        ft.Container(
                            content=ft.Column([
                                ft.Icon(ft.Icons.FLAG, color=flag_color, size=20),
                                ft.Text(f"{flagged}", color=flag_color, weight="bold")
                    ...
                    padding=20,
                    on_click=lambda e, i=idx: open_status_view(i)
                )
        ...
        def on_scroll_list(e: ft.OnScrollEvent):
            state["scroll_pos"] = e.pixels

        list_container = ft.ListView(
            expand=True, 
            spacing=10, 
            on_scroll=on_scroll_list 
        )
        ...
        def render_jobs(filter_text=""):
            ...
            if not filter_text:
                ...
                for j in jobs:
                    ...
                    if filter_text in full_text:
                        visible_jobs.append(j)

            if not visible_jobs:
                list_container.controls.append(ft.Text("No jobs match filter.", text_align="center"))
            
            for job in visible_jobs:
                flag_val = job.get('flagged', 0)
                is_flagged = (flag_val == 1) or (flag_val is True)
                
                if is_flagged:
                    card_bg = ft.Colors.RED_400
                    text_color = "white"
                ...
                raw_date = str(job.get('created_at', ''))
                nice_date = raw_date[:10] if raw_date else ""

                card_content = ft.Container(
                    content=ft.Column([
                        ft.Row([
                            ft.Text(job['job_code'], weight="bold", size=16, color=text_color),
                            ft.Text(nice_date, size=12, color=sub_text_color)
                        ], alignment="spaceBetween"),
        ...
        page.add(
            ft.Container(
                content=ft.Column([
                    ft.Container(content=local_search_box, padding=10), 
                    list_container 
    ...
    def show_job_details(job):
        ...
        t_code = ft.TextField(label="Job Code", value=job['job_code'] if job else f"JOB-{int(datetime.now().timestamp())}", disabled=(not is_new))
        ...
        def save_click(e):
            ...
            data = {
                ...
                "customer": t_cust.value,
                "supervisor": t_pic.value,
                "trailer_type": t_type.value,
                "price_text": t_price.value,
            ...
            if is_new:
                ...
                ok = db.create_job(data, state["user"])
            else:
                ok = db.update_job(job['id'], data, state["user"], job['job_code'])

            if ok:
                reload_current_view()
            else:
                page.snack_bar = ft.SnackBar(ft.Text("Error saving!"))
        ...
        def move_status_click(target_idx):
            ...
            db.update_job(job['id'], updates, state["user"], job['job_code'], 
                          old_status=job['status_idx'], new_status=target_idx)
        ...
        page.appbar = ft.AppBar(
            ...
            title=ft.Text("Job Details"),
            bgcolor="blue",
            color="white",
            actions=[
                ft.IconButton(ft.Icons.CHECK, on_click=save_click) if not is_readonly else ft.Container()
            ]
        )
        ...
        if not is_new and not is_readonly:
            ...
            for idx, label in STATUSES:
                if idx < 7 and idx != current_s:
                    # CHANGED: NO MORE CLEANING/CUTTING. Use the full label as is.
                    btn = ft.OutlinedButton(label, on_click=lambda e, i=idx: move_status_click(i))
                    row_btns.append(btn)
        ...
        page.add(
            ft.Container(
                content=ft.Column(
                    [
                        t_code, t_cust, t_pic, t_summary, t_type, t_price, c_flag, t_notes,
                        ft.Divider(),
    ...
    def load_history_view():
        ...
        for log in logs:
            ...
            time = str(log.get('changed_at', ''))[:16]
